...
def test_generic_adam_initialization():
    """
    Test the initialization of the GenericAdam optimizer.
    """
    model = nn.Linear(10, 1)  # Simple model with 10 inputs, 1 output
    optimizer = GenericAdam(model.parameters(), lr=0.01, beta=0.9, r=0.5, epsilon=1e-8)

    assert optimizer.defaults["lr"] == 0.01
    assert optimizer.defaults["beta"] == 0.9
    assert optimizer.defaults["r"] == 0.5
    assert optimizer.defaults["epsilon"] == 1e-8


def test_generic_adam_step():
    """
    ...
    optimizer = GenericAdam(model.parameters(), lr=0.1)
...
def test_generic_adam_training():
    """
    Test if the GenericAdam optimizer can successfully optimize a small regression problem.
    """
    torch.manual_seed(42)
    ...
    criterion = nn.MSELoss()
    optimizer = GenericAdam(model.parameters(), lr=0.01, r=0.5)
