...
def test_rmsprop_builtin_initialization():
    """
    Test the initialization of the RMSPropBuiltin optimizer.
    """
    model = nn.Linear(10, 1)  # Simple model with 10 inputs, 1 output
    optimizer = RMSPropBuiltin(model.parameters(), alpha=0.01, beta=0.9, epsilon=1e-8)

    # Check optimizer parameters
    assert optimizer.base_alpha == 0.01
    assert optimizer.beta == 0.9
    assert optimizer.epsilon == 1e-8


def test_rmsprop_builtin_step():
    """
    ...
    optimizer = RMSPropBuiltin(model.parameters(), alpha=0.1, beta=0.9, epsilon=1e-8)
...
def test_rmsprop_builtin_training():
    """
    Test if the RMSPropBuiltin optimizer can successfully optimize a small regression problem.
    """
    torch.manual_seed(42)
