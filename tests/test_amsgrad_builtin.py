...
def test_amsgrad_builtin_initialization():
    """
    Test the initialization of the AMSGradBuiltin optimizer.
    """
    from optimizers.amsgrad_builtin import AMSGradBuiltin

    model = nn.Linear(10, 1)
    optimizer = AMSGradBuiltin(model.parameters(), lr=0.01, betas=(0.9, 0.999), eps=1e-8, amsgrad=True)

    # Check optimizer parameters
    assert optimizer.base_lr == 0.01
    assert optimizer.optimizer.defaults["betas"] == (0.9, 0.999)
    assert optimizer.optimizer.defaults["eps"] == 1e-8


def test_amsgrad_builtin_step():
    """
    ...
    optimizer = AMSGradBuiltin(model.parameters(), lr=0.1, betas=(0.9, 0.999), eps=1e-8, amsgrad=True)
...
def test_amsgrad_builtin_training():
    """
    Test if the AMSGradBuiltin optimizer can successfully optimize a small regression problem.
    """
    from optimizers.amsgrad_builtin import AMSGradBuiltin
